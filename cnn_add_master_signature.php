...
if ($success === false) {
    ...
    echo "<script>location.href='maincalibration.php?p=sign_master'</script>";
} else {
    $newname2 =  $newname . '.png';
    include "../database.php";
    $sql = "UPDATE userlogin SET signature = '$newname2' WHERE userid = $userid";
    $result = sqlsrv_query($conn, $sql);
    if ($result === false) {
        echo "<script>alert('FAILED TO QUERY UPDATE TO DATABASE');</script>";
        echo "<script>location.href='maincalibration.php?p=sign_master'</script>";
    } else {
        $_SESSION['cb_signature'] = $newname2;
        echo "<script>alert('เพิ่มลายเซ็นสำเร็จ');</script>";
        if ($_SESSION['cb_type'] == 1) {
            echo "<script>location.href='maincalibration.php?p=superadmin_dashboard'</script>";
        }
        if ($_SESSION['cb_type'] == 2) {
            echo "<script>location.href='maincalibration.php?p=dashboard'</script>";
        }
